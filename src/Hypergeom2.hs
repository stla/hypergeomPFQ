...
module Hypergeom2 (hypergeom) where
...
import           Shared        hiding (hypergeoI, _T, _betaratio)
...
instance Num a => BaseFrac (Complex a) where
  ...
  inject x = x :+ 0
-- instance BaseFrac (Complex Rational) where
--   type BaseFracType (Complex Rational) = Rational
--   inject x = x :+ 0
